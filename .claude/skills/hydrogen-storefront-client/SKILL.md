...
function getBuyerIp(headers: Headers) {
  ...
  if (!buyerIp)
    throw new Error("oxygen-buyer-ip is required for private SFAPI clients");
  return buyerIp;
}
...
const PRODUCT_FIELDS = gql(
  `fragment ProductFields on Product { title handle }`,
);
const QUERY = gql(
...
try {
  ...
  if (errors) {
    /* partial GraphQL errors — query succeeded but some fields failed */
  }
} catch (error) {
  ...
  if (error instanceof StorefrontApiError) {
    /* non-200, network failure, or JSON parse error */
  }
}
```
